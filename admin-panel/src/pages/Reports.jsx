...
const S = {
  ...
  preset:    (active) => ({ background: active?"#3b82f6":"#fff", color:active?"#fff":"#374151", border:"1px solid "+(active?"#3b82f6":"#e2e8f0"), borderRadius:8, padding:"8px 14px", cursor:"pointer", fontSize:13, fontWeight:500 }),
  ...
  row3:      { display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:20, marginBottom:24 },
...
export default function Reports() {
  ...
  const hourBuckets = Array.from({ length: 24 }, (_, h) => {
    const secs = activity.filter(a => {
      if (!a.start_time) return false;
      return new Date(a.start_time).getHours() === h;
    ...
    <div>
      ...
      <div style={S.row2}>
        ...
        <div style={S.card}>
          ...
          {appSummary.length === 0
            ? <div style={{ color:"#94a3b8", fontSize:14, paddingTop:60, textAlign:"center" }}>No app data for this day.</div>
            : (
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie data={appSummary.slice(0,6).map(a => ({ name:a.app_name, value:a.total_seconds }))}
                    dataKey="value" cx="50%" cy="50%" outerRadius={80}
                    label={({ name, percent }) => `${name.slice(0,10)} ${(percent*100).toFixed(0)}%`}
                    labelLine={false}>
                    {appSummary.slice(0,6).map((_,i) => <Cell key={i} fill={COLORS[i%COLORS.length]} />)}
                  </Pie>
      ...
      <div style={{ ...S.card, marginBottom:24 }}>
        ...
        <ResponsiveContainer width="100%" height={120}>
          <BarChart data={hourBuckets} margin={{ top:0, bottom:0 }}>
            ...
            <YAxis hide />
            <Tooltip formatter={v => fmtDur(v)} />
            <Bar dataKey="secs" fill="#3b82f6" radius={[2,2,0,0]}>
      ...
      <div style={S.row2}>
          ...
          {appSummary.map((a, i) => (
            <div key={a.app_name} style={{ marginBottom:10 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                ...
                <span style={{ fontSize:12, color:"#6b7280" }}>{fmtDur(a.total_seconds)} ({totalSecs > 0 ? Math.round(a.total_seconds/totalSecs*100) : 0}%)</span>
              </div>
              <div style={{ background:"#f3f4f6", borderRadius:4, height:6 }}>
                <div style={{ height:6, borderRadius:4, background:COLORS[i%COLORS.length], width: totalSecs > 0 ? `${(a.total_seconds/totalSecs*100).toFixed(1)}%` : "0%" }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={S.card}>
          ...
          <table style={S.table}>
            <thead>
              <tr>
                {["Employee","Punch In","Punch Out","Duration","Status"].map(h => <th key={h} style={S.th}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {sessions.map(s => {
                ...
                return (
                  <tr key={s.id}>
                    ...
                    <td style={S.td}>{s.punch_out ? format(new Date(s.punch_out), "h:mm a") : "—"}</td>
                    <td style={S.td}>{fmtHM(s.total_minutes || 0)}</td>
                    <td style={S.td}>
        ...
        <table style={S.table}>
          ...
          <tbody>
            {activity.slice(0, 50).map(log => {
              ...
              return (
                <tr key={log.id}>
                  <td style={{ ...S.td, fontFamily:"monospace", fontSize:12, color:"#9ca3af" }}>{log.start_time ? format(new Date(log.start_time), "h:mm a") : "—"}</td>
                  ...
                  <td style={S.td}>{fmtDur(log.duration_seconds || 0)}</td>
                </tr>
              );
            })}
          </tbody>
